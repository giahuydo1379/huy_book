...
NetBase.extend({
	...
	ajaxSettings: {
		...
		*/
		...
		converters: {
			...
			"text json": NetBase.parseJSON,

			// Parse text as xml
			"text xml": NetBase.parseXML
		},
	...
	ajax: function( url, options ) {
		...
		var // Create the final options object
			s = NetBase.ajaxSetup( {}, options ),
			// Callbacks context
			callbackContext = s.context || s,
			// Context for global events
			// It's the callbackContext if one was provided in the options
			// and if it's a DOM node or a NetBase collection
			globalEventContext = callbackContext !== s &&
				( callbackContext.nodeType || callbackContext instanceof NetBase ) ?
						NetBase( callbackContext ) : NetBase.event,
			// Deferreds
			deferred = NetBase.Deferred(),
			completeDeferred = NetBase.Callbacks( "once memory" ),
		...
		function done( status, nativeStatusText, responses, headers ) {
			...
			if ( status >= 200 && status < 300 || status === 304 ) {
				...
				if ( s.ifModified ) {
					...
					if ( ( lastModified = jqXHR.getResponseHeader( "Last-Modified" ) ) ) {
						NetBase.lastModified[ ifModifiedKey ] = lastModified;
					}
					if ( ( etag = jqXHR.getResponseHeader( "Etag" ) ) ) {
						NetBase.etag[ ifModifiedKey ] = etag;
					}
				}
			...
			if ( fireGlobals ) {
				...
				if ( !( --NetBase.active ) ) {
					NetBase.event.trigger( "ajaxStop" );
				}
			}
		}
		...
		s.dataTypes = NetBase.trim( s.dataType || "*" ).toLowerCase().split( rspacesAjax );
		...
		if ( s.data && s.processData && typeof s.data !== "string" ) {
			s.data = NetBase.param( s.data, s.traditional );
		}
		...
		if ( fireGlobals && NetBase.active++ === 0 ) {
			NetBase.event.trigger( "ajaxStart" );
		}
		...
		if ( s.ifModified ) {
			...
			if ( NetBase.lastModified[ ifModifiedKey ] ) {
				jqXHR.setRequestHeader( "If-Modified-Since", NetBase.lastModified[ ifModifiedKey ] );
			}
			if ( NetBase.etag[ ifModifiedKey ] ) {
				jqXHR.setRequestHeader( "If-None-Match", NetBase.etag[ ifModifiedKey ] );
			}
		}
	...
	param: function( a, traditional ) {
		var s = [],
			add = function( key, value ) {
				...
				value = NetBase.isFunction( value ) ? value() : value;
				s[ s.length ] = encodeURIComponent( key ) + "=" + encodeURIComponent( value );
			};

		// Set traditional to true for NetBase <= 1.3.2 behavior.
		if ( traditional === undefined ) {
			traditional = NetBase.ajaxSettings.traditional;
		}

		// If an array was passed in, assume that it is an array of form elements.
		if ( NetBase.isArray( a ) || ( a.netbase && !NetBase.isPlainObject( a ) ) ) {
			// Serialize the form elements
			NetBase.each( a, function() {
...
function buildParams( prefix, obj, traditional, add ) {
	if ( NetBase.isArray( obj ) ) {
		// Serialize array item.
		NetBase.each( obj, function( i, v ) {
	...
	} else if ( !traditional && NetBase.type( obj ) === "object" ) {
...
// This is still on the NetBase object... for now
// Want to move this to NetBase.ajax some day
NetBase.extend({
...
function ajaxConvert( s, response ) {
	...
	for ( i = 1; i < length; i++ ) {
		...
		// If no auto and dataTypes are actually different
			...
			if ( !( conv || conv2 ) ) {
				NetBase.error( "No conversion from " + conversion.replace(" "," to ") );
			}
...
NetBase.ajaxSetup({
	...
	jsonpCallback: function() {
		return NetBase.expando + "_" + ( jsc++ );
	}
});

// Detect, normalize options and install callbacks for jsonp requests
NetBase.ajaxPrefilter( "json jsonp", function( s, originalSettings, jqXHR ) {
	...
	if ( s.dataTypes[ 0 ] === "jsonp" ||
		...
		var responseContainer,
			jsonpCallback = s.jsonpCallback =
				NetBase.isFunction( s.jsonpCallback ) ? s.jsonpCallback() : s.jsonpCallback,
		...
		jqXHR.always(function() {
			...
			if ( responseContainer && NetBase.isFunction( previous ) ) {
		...
		s.converters["script json"] = function() {
			if ( !responseContainer ) {
				NetBase.error( jsonpCallback + " was not called" );
			}
...
NetBase.ajaxPrefilter( "script", function( s ) {
...
NetBase.ajaxTransport( "script", function(s) {
...
NetBase.ajaxSettings.xhr = window.ActiveXObject ?
...
(function( xhr ) {
	NetBase.extend( NetBase.support, {
		ajax: !!xhr,
		cors: !!xhr && ( "withCredentials" in xhr )
	});
})( NetBase.ajaxSettings.xhr() );

// Create transport if the browser can provide an xhr
if ( NetBase.support.ajax ) {

	NetBase.ajaxTransport(function( s ) {
		// Cross domain only allowed if supported through XMLHttpRequest
		if ( !s.crossDomain || NetBase.support.cors ) {
			...
			return {
				send: function( headers, complete ) {
					...
					// handled in NetBase.ajax (so no try/catch here)
					...
					callback = function( _, isAbort ) {
						...
						try {
							...
							if ( callback && ( isAbort || xhr.readyState === 4 ) ) {
								...
								if ( handle ) {
									xhr.onreadystatechange = NetBase.noop;
									if ( xhrOnUnloadAbort ) {
					...
					if ( !s.async || xhr.readyState === 4 ) {
						...
						if ( xhrOnUnloadAbort ) {
							...
							if ( !xhrCallbacks ) {
								xhrCallbacks = {};
								NetBase( window ).unload( xhrOnUnloadAbort );
							}
...
NetBase.fn.extend({
		...
		if ( speed || speed === 0 ) {
			...
			for ( var i = 0, j = this.length; i < j; i++ ) {
				...
				if ( elem.style ) {
					...
					if ( !NetBase._data(elem, "olddisplay") && display === "none" ) {
						display = elem.style.display = "";
					}
					...
					// for such an element
					if ( (display === "" && NetBase.css(elem, "display") === "none") ||
						!NetBase.contains( elem.ownerDocument.documentElement, elem ) ) {
						NetBase._data( elem, "olddisplay", defaultDisplay(elem.nodeName) );
					}
				}
			}
			...
			for ( i = 0; i < j; i++ ) {
				...
				if ( elem.style ) {
					...
					if ( display === "" || display === "none" ) {
						elem.style.display = NetBase._data( elem, "olddisplay" ) || "";
					}
				}
			}
	...
	hide: function( speed, easing, callback ) {
		if ( speed || speed === 0 ) {
			...
			for ( ; i < j; i++ ) {
				elem = this[i];
				if ( elem.style ) {
					display = NetBase.css( elem, "display" );

					if ( display !== "none" && !NetBase._data( elem, "olddisplay" ) ) {
						NetBase._data( elem, "olddisplay", display );
					}
				}
			}
	...
	_toggle: NetBase.fn.toggle,

	toggle: function( fn, fn2, callback ) {
		var bool = typeof fn === "boolean";

		if ( NetBase.isFunction(fn) && NetBase.isFunction(fn2) ) {
			this._toggle.apply( this, arguments );

		} else if ( fn == null || bool ) {
			this.each(function() {
				var state = bool ? fn : NetBase(this).is(":hidden");
				NetBase(this)[ state ? "show" : "hide" ]();
			});
	...
	animate: function( prop, speed, easing, callback ) {
		var optall = NetBase.speed( speed, easing, callback );

		if ( NetBase.isEmptyObject( prop ) ) {
			return this.each( optall.complete, [ false ] );
		}

		// Do not change referenced properties as per-property easing will be lost
		prop = NetBase.extend( {}, prop );
		...
		function doAnimation() {
			...
			var opt = NetBase.extend( {}, optall ),
				isElement = this.nodeType === 1,
				hidden = isElement && NetBase(this).is(":hidden"),
			...
			for ( p in prop ) {
				name = NetBase.camelCase( p );
				if ( p !== name ) {
					prop[ name ] = prop[ p ];
					delete prop[ p ];
				}

				if ( ( hooks = NetBase.cssHooks[ name ] ) && "expand" in hooks ) {
			...
			for ( name in prop ) {
				...
				if ( isElement && ( name === "height" || name === "width" ) ) {
					...
					if ( NetBase.css( this, "display" ) === "inline" &&
							NetBase.css( this, "float" ) === "none" ) {

						// inline-level elements accept inline-block;
						// block-level elements need to be inline with layout
						if ( !NetBase.support.inlineBlockNeedsLayout || defaultDisplay( this.nodeName ) === "inline" ) {
			...
			for ( p in prop ) {
				...
				if ( rfxtypes.test( val ) ) {
					...
					method = NetBase._data( this, "toggle" + p ) || ( val === "toggle" ? hidden ? "show" : "hide" : 0 );
					if ( method ) {
						NetBase._data( this, "toggle" + p, method === "show" ? "hide" : "show" );
						e[ method ]();
					} else {
						e[ val ]();
					}
					...
					if ( parts ) {
						...
						unit = parts[3] || ( NetBase.cssNumber[ p ] ? "" : "px" );

						// We need to compute starting value
						if ( unit !== "px" ) {
							NetBase.style( this, p, (end || 1) + unit);
							start = ( (end || 1) / e.cur() ) * start;
							NetBase.style( this, p, start + unit);
						}
	...
	stop: function( type, clearQueue, gotoEnd ) {
		...
		return this.each(function() {
			var index,
				hadTimers = false,
				timers = NetBase.timers,
				data = NetBase._data( this );

			// clear marker counters if we know they won't be
			if ( !gotoEnd ) {
				NetBase._unmark( true, this );
			}

			function stopQueue( elem, data, index ) {
				var hooks = data[ index ];
				NetBase.removeData( elem, index, true );
				hooks.stop( gotoEnd );
			}
...
function genFx( type, num ) {
	...
	NetBase.each( fxAttrs.concat.apply([], fxAttrs.slice( 0, num )), function() {
		obj[ this ] = type;
	});
...
NetBase.each({
	...
	NetBase.fn[ name ] = function( speed, easing, callback ) {
...
NetBase.extend({
	speed: function( speed, easing, fn ) {
		var opt = speed && typeof speed === "object" ? NetBase.extend( {}, speed ) : {
			complete: fn || !fn && easing ||
				NetBase.isFunction( speed ) && speed,
			duration: speed,
			easing: fn && easing || easing && !NetBase.isFunction( easing ) && easing
		};

		opt.duration = NetBase.fx.off ? 0 : typeof opt.duration === "number" ? opt.duration :
			opt.duration in NetBase.fx.speeds ? NetBase.fx.speeds[ opt.duration ] : NetBase.fx.speeds._default;
		...
		opt.complete = function( noUnmark ) {
			if ( NetBase.isFunction( opt.old ) ) {
				opt.old.call( this );
			}

			if ( opt.queue ) {
				NetBase.dequeue( this, opt.queue );
			} else if ( noUnmark !== false ) {
				NetBase._unmark( this );
			}
		};
...
NetBase.fx.prototype = {
	...
	update: function() {
		...
		( NetBase.fx.step[ this.prop ] || NetBase.fx.step._default )( this );
	},
	...
	cur: function() {
		...
		var parsed,
			r = NetBase.css( this.elem, this.prop );
	...
	custom: function( from, to, unit ) {
		...
		this.unit = unit || this.unit || ( NetBase.cssNumber[ this.prop ] ? "" : "px" );
		...
		t.saveState = function() {
			if ( NetBase._data( self.elem, "fxshow" + self.prop ) === undefined ) {
				if ( self.options.hide ) {
					NetBase._data( self.elem, "fxshow" + self.prop, self.start );
				} else if ( self.options.show ) {
					NetBase._data( self.elem, "fxshow" + self.prop, self.end );
				}
			}
		};

		if ( t() && NetBase.timers.push(t) && !timerId ) {
	...
	show: function() {
		var dataShow = NetBase._data( this.elem, "fxshow" + this.prop );

		// Remember where we started, so that we can go back to it later
		this.options.orig[ this.prop ] = dataShow || NetBase.style( this.elem, this.prop );
		...
		NetBase( this.elem ).show();
	},

	// Simple 'hide' function
	hide: function() {
		// Remember where we started, so that we can go back to it later
		this.options.orig[ this.prop ] = NetBase._data( this.elem, "fxshow" + this.prop ) || NetBase.style( this.elem, this.prop );
	...
	step: function( gotoEnd ) {
		...
		if ( gotoEnd || t >= options.duration + this.startTime ) {
			...
			if ( done ) {
				// Reset the overflow
				if ( options.overflow != null && !NetBase.support.shrinkWrapBlocks ) {

					NetBase.each( [ "", "X", "Y" ], function( index, value ) {
				...
				if ( options.hide || options.show ) {
					for ( p in options.animatedProperties ) {
						NetBase.style( elem, p, options.orig[ p ] );
						NetBase.removeData( elem, "fxshow" + p, true );
						// Toggle data is no longer needed
						NetBase.removeData( elem, "toggle" + p, true );
					}
				}
			...
			if ( options.duration == Infinity ) {
				...
				this.pos = NetBase.easing[ options.animatedProperties[this.prop] ]( this.state, n, 0, 1, options.duration );
...
NetBase.extend( NetBase.fx, {
	...
	step: {
		opacity: function( fx ) {
			NetBase.style( fx.elem, "opacity", fx.now );
		},
...
NetBase.each( fxAttrs.concat.apply( [], fxAttrs ), function( i, prop ) {
	// exclude marginTop, marginLeft, marginBottom and marginRight from this list
	if ( prop.indexOf( "margin" ) ) {
		NetBase.fx.step[ prop ] = function( fx ) {
			NetBase.style( fx.elem, prop, Math.max(0, fx.now) + fx.unit );
		};
	}
});

if ( NetBase.expr && NetBase.expr.filters ) {
	NetBase.expr.filters.animated = function( elem ) {
		return NetBase.grep(NetBase.timers, function( fn ) {
			return elem === fn.elem;
		}).length;
	};
}
...
function defaultDisplay( nodeName ) {
	...
	if ( !elemdisplay[ nodeName ] ) {
		...
		var body = document.body,
			elem = NetBase( "<" + nodeName + ">" ).appendTo( body ),
		...
		if ( display === "none" || display === "" ) {
			...
			if ( !iframeDoc || !iframe.createElement ) {
				...
				iframeDoc.write( ( NetBase.support.boxModel ? "<!doctype html>" : "" ) + "<html><body>" );
				iframeDoc.close();
			}
...
if ( "getBoundingClientRect" in document.documentElement ) {
	getOffset = function( elem, doc, docElem, box ) {
		...
		if ( !box || !NetBase.contains( docElem, elem ) ) {
		...
		var body = doc.body,
			...
			scrollTop  = win.pageYOffset || NetBase.support.boxModel && docElem.scrollTop  || body.scrollTop,
			scrollLeft = win.pageXOffset || NetBase.support.boxModel && docElem.scrollLeft || body.scrollLeft,
	...
	getOffset = function( elem, doc, docElem ) {
		...
		while ( (elem = elem.parentNode) && elem !== body && elem !== docElem ) {
			if ( NetBase.support.fixedPosition && prevComputedStyle.position === "fixed" ) {
				break;
			}
			...
			if ( elem === offsetParent ) {
				...
				if ( NetBase.support.doesNotAddBorder && !(NetBase.support.doesAddBorderForTableAndCells && rtable.test(elem.nodeName)) ) {
			...
			if ( NetBase.support.subtractsBorderForOverflowNotVisible && computedStyle.overflow !== "visible" ) {
				top  += parseFloat( computedStyle.borderTopWidth  ) || 0;
				left += parseFloat( computedStyle.borderLeftWidth ) || 0;
			}
...
NetBase.fn.offset = function( options ) {
	if ( arguments.length ) {
		return options === undefined ?
			this :
			this.each(function( i ) {
				NetBase.offset.setOffset( this, options, i );
			});
	}
	...
	if ( elem === doc.body ) {
		return NetBase.offset.bodyOffset( elem );
	}
...
NetBase.offset = {
	...
	bodyOffset: function( body ) {
		...
		if ( NetBase.support.doesNotIncludeMarginInBodyOffset ) {
			top  += parseFloat( NetBase.css(body, "marginTop") ) || 0;
			left += parseFloat( NetBase.css(body, "marginLeft") ) || 0;
		}
	...
	setOffset: function( elem, options, i ) {
		var position = NetBase.css( elem, "position" );
		...
		var curElem = NetBase( elem ),
			curOffset = curElem.offset(),
			curCSSTop = NetBase.css( elem, "top" ),
			curCSSLeft = NetBase.css( elem, "left" ),
			calculatePosition = ( position === "absolute" || position === "fixed" ) && NetBase.inArray("auto", [curCSSTop, curCSSLeft]) > -1,
...
NetBase.fn.extend({
	...
	position: function() {
		...
		offset.top  -= parseFloat( NetBase.css(elem, "marginTop") ) || 0;
		offset.left -= parseFloat( NetBase.css(elem, "marginLeft") ) || 0;

		// Add offsetParent borders
		parentOffset.top  += parseFloat( NetBase.css(offsetParent[0], "borderTopWidth") ) || 0;
		parentOffset.left += parseFloat( NetBase.css(offsetParent[0], "borderLeftWidth") ) || 0;
	...
	offsetParent: function() {
		return this.map(function() {
			...
			while ( offsetParent && (!rroot.test(offsetParent.nodeName) && NetBase.css(offsetParent, "position") === "static") ) {
...
NetBase.each( {scrollLeft: "pageXOffset", scrollTop: "pageYOffset"}, function( method, prop ) {
	var top = /Y/.test( prop );

	NetBase.fn[ method ] = function( val ) {
		return NetBase.access( this, function( elem, method, val ) {
			var win = getWindow( elem );

			if ( val === undefined ) {
				return win ? (prop in win) ? win[ prop ] :
					NetBase.support.boxModel && win.document.documentElement[ method ] ||
			...
			if ( win ) {
				win.scrollTo(
					!top ? val : NetBase( win ).scrollLeft(),
					 top ? val : NetBase( win ).scrollTop()
				);
...
NetBase.each( { Height: "height", Width: "width" }, function( name, type ) {
	var clientProp = "client" + name,
		scrollProp = "scroll" + name,
		offsetProp = "offset" + name;

	// innerHeight and innerWidth
	NetBase.fn[ "inner" + name ] = function() {
		var elem = this[0];
		return elem ?
			elem.style ?
			parseFloat( NetBase.css( elem, type, "padding" ) ) :
			this[ type ]() :
			null;
	};

	// outerHeight and outerWidth
	NetBase.fn[ "outer" + name ] = function( margin ) {
		var elem = this[0];
		return elem ?
			elem.style ?
			parseFloat( NetBase.css( elem, type, margin ? "margin" : "border" ) ) :
			this[ type ]() :
			null;
	};

	NetBase.fn[ type ] = function( value ) {
		return NetBase.access( this, function( elem, type, value ) {
			var doc, docElemProp, orig, ret;

			if ( NetBase.isWindow( elem ) ) {
				// 3rd condition allows Nokia support, as it supports the docElem prop but not CSS1Compat
				doc = elem.document;
				docElemProp = doc.documentElement[ clientProp ];
				return NetBase.support.boxModel && docElemProp ||
			...
			if ( value === undefined ) {
				orig = NetBase.css( elem, type );
				ret = parseFloat( orig );
				return NetBase.isNumeric( ret ) ? ret : orig;
			}

			// Set the width or height on the element
			NetBase( elem ).css( type, value );
...
// Expose NetBase to the global object
window.NetBase = window.$ = NetBase;

// Expose NetBase as an AMD module, but only for AMD loaders that
// understand the issues with loading multiple versions of NetBase
// in a page that all might call define(). The loader will indicate
// they have special allowances for multiple NetBase versions by
// specifying define.amd.NetBase = true. Register as a named module,
// since NetBase can be concatenated with other files that may use define,
// but not use a proper concatenation script that understands anonymous
// AMD modules. A named AMD is safest and most robust way to register.
// Lowercase NetBase is used because AMD module names are derived from
// file names, and NetBase is normally delivered in a lowercase file name.
// Do this after creating the global so that if an AMD module wants to call
// noConflict to hide this version of NetBase, it will work.
if ( typeof define === "function" && define.amd && define.amd.NetBase ) {
	define( "netbase", [], function () { return NetBase; } );
}
