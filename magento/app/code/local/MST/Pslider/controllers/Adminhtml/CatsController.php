 ...
 */
class MST_Pslider_Adminhtml_CatsController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        $this->_initAction();
        $this->_addContent($this->getLayout()->createBlock('pslider/adminhtml_cats'));
        $this->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        $model = Mage::getModel('pslider/cats')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this->_addContent($this->getLayout()->createBlock('pslider/adminhtml_cats_edit'))
                ->_addLeft($this->getLayout()->createBlock('pslider/adminhtml_cats_edit_tabs'));
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (isset($_FILES['filename']['name']) && $_FILES['filename']['name'] != '') {
                try {
                    ...
                    $path = Mage::getBaseDir('media') . DS . 'pslider' . DS;
            ...
            $model = Mage::getModel('pslider/cats');
            $model->setData($data)
            ...
            try {
				...
				$model1 = Mage::getModel('pslider/cats')->load($model->getCatsId());
				$installGuide = Mage::getModel('pslider/cats')->installGuide( $model->getCatsId() , $model->getLayout() );
				$model1->setInfo($installGuide);
				$model1->save();
        ...
        if (!is_array($psliderIds)) {
            ...
            try {
                foreach ($psliderIds as $psliderId) {
                    $pslider = Mage::getModel('pslider/cats')->load($psliderId);
                    $pslider->delete();
                }
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($psliderIds)) {
            ...
            try {
                foreach ($psliderIds as $psliderId) {
                    $pslider = Mage::getSingleton('pslider/cats')
                            ->load($psliderId)
